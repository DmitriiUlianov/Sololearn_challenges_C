...
The total number of carrots in each box doesn't matter as long as you distribute them evenly, and there are not enough leftover to put another carrot in each box.
...
int main() {
    ...
    int carrots, boxes;
    scanf("%d%d", &carrots, &boxes);
    int res = 0;
    res = carrots % boxes;
    int x = 7 - res;
    if (x>0)
        printf ("I need to buy %d more", x);
    else 
        printf ("Cake Time");
    return 0;
}
